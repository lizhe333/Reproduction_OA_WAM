...
- 不追求一开始复现论文数值，先追求机制正确：forward、OA invariant、address swap diagnostic。
...
- Backbone 使用现有 Chameleon-style/VLM backbone，不复现 Stage 0 预训练。
- Stage 0: 跳过，使用现有 backbone 或 released checkpoint。
...
- `HeadsTraining`: 实现 world/action heads、loss、Stage I/II loop。
